...
async function request<T>(path: string, init: RequestInit = {}, auth = true): Promise<T> {
  ...
  catch { throw new ApiError(0, "Could not reach the Lotwise Core API. Confirm it is running at the configured URL."); }
  if (!response.ok) {
...
export const api = {
  ...
  products: () => request<{ items: Product[] }>("/v1/products"), facilities: () => request<{ items: Facility[] }>("/v1/facilities"),
  bundle: (id: string) => request<Bundle>(`/v1/products/${id}`), runs: (id: string) => request<{ items: Run[] }>(`/v1/screenings?product_id=${id}`), run: (id: string) => request<Run>(`/v1/screenings/${id}`),
